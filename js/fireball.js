(function() {
  ...
  Fireball.prototype.update = function(dt) {
    if (this.hit == 1) {
      ...
      // remove the fireball; only decrement player's fireball counter
      // if this was spawned by the player
      delete fireballs[this.idx];
      if (this._ownerPlayer) player.fireballs -= 1;
      return;
    } else if (this.hit) {
      this.hit += 1;
      return;
  ...
  Fireball.prototype.isCollideWith = function(ent) {
    ...
    if (!(hpos1[0] > hpos2[0]+ent.hitbox[2] || (hpos1[0]+this.hitbox[2] < hpos2[0]))) {
      if (!(hpos1[1] > hpos2[1]+ent.hitbox[3] || (hpos1[1]+this.hitbox[3] < hpos2[1]))) {
        this.hit = 1;
        // if this fireball belongs to the player, bump the enemy
        if (this._ownerPlayer) {
          ent.bump();
        } else {
          // enemy fireballs damage the player
          if (ent instanceof Mario.Player) {
            ent.takeEnemyFireballHit();
          }
        }
      }
    }
