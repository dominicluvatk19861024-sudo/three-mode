(function() {
	...
	var Player = Mario.Player = function(pos) {
		...
		this.fireballHits = 0; // number of times hit by enemy fireballs
		this.doubleJumpPressed = false;
		this.invincibility = 0;
		this.invisible = false;
		this.invisiblePressed = false;
	...
	Player.prototype.doubleJump = function() {
			...
			// Give a stronger upward impulse and slightly longer jump duration
			this.jumping = 28;
			this.canDoubleJump = false;
			this.standing = false;
			this.vel[1] = -9; // stronger negative velocity = higher jump
		if (this.power === 0) {
			sounds.smallJump.currentTime = 0;
			sounds.smallJump.play();
		} else {
  ...
  Player.prototype.setAnimation = function() {
	...
	Player.prototype.takeEnemyFireballHit = function() {
		// ignore if currently invincible
		if (this.invincibility) return;
		this.fireballHits = (this.fireballHits || 0) + 1;
	 	// small invulnerability window so two simultaneous fireballs don't stack instantly
	 	this.invincibility = 60;
	 	if (this.fireballHits >= 2) {
	 		this.die();
	 	} else {
