(function() {
  ...
  var Goomba = Mario.Goomba = function(pos, sprite) {
    ...
    this.shootCounter = Math.random() * 3 + 1; // start offset
  };
  ...
  Goomba.prototype.update = function(dt, vX) {
    ...
    // floating behavior if para flag is set
    if (this.para) {
      // simple sine-based bobbing by adjusting vel[1]
      if (this._bob === undefined) this._bob = Math.random() * 2 * Math.PI;
      this._bob += dt * 3;
      this.pos[1] += Math.sin(this._bob) * 0.6;
    } else {
      this.acc[1] = 0.2;
    ...
    // enemy shooting (periodic)
    this.shootCounter -= dt;
    if (this.shootCounter <= 0) {
      this.shootCounter = 4 + Math.random() * 3; // every 4-7s
      this.shoot();
    }
  };

  Goomba.prototype.shoot = function() {
    // create a red fireball that moves in the goomba's facing direction
    var left = this.vel[0] < 0;
    var fb = new Mario.Fireball([this.pos[0] + (left ? -4 : 12), this.pos[1] + 4]);
    fb._ownerPlayer = false;
    fb.spawn(left);
    // make the enemy fireball visually different (optional): use a different sprite frame
    fb.sprite.pos = [96, 144];
  };
