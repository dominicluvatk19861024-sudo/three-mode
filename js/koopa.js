(function() {
  ...
  var Koopa = Mario.Koopa = function(pos, sprite, para) {
    ...
    this.shootCounter = Math.random() * 3 + 1;
  };
  ...
  Koopa.prototype.update = function(dt, vX) {
    ...
    // periodic shooting for koopas
    this.shootCounter -= dt;
    if (this.shootCounter <= 0) {
      this.shootCounter = 3 + Math.random() * 4; // every 3-7s
      this.shoot();
    }
  };
  ...
  Koopa.prototype.shoot = function() {
    ...
    var fb = new Mario.Fireball([this.pos[0] + (left ? -4 : 18), this.pos[1] + 8]);
